...
padding_length = args.padding_length if 'padding_length' in args._dict else 0
horizon_HL =  args.seg_length + padding_length
...
for t in range(env.max_episode_steps):
    ...
    if t == 0:
        ...
        for idx in range(args.seg_length):
            cond_plot[(args.LL_horizon - 1) * idx] = hl_goal[idx]
        ...
        LL_cond = {}
        # for idx in range(seg_length):
        #     LL_cond[(args.LL_horizon - 1) * idx] = hl_goal[idx]

        LL_cond[0] = hl_goal[0]
        LL_cond[(args.LL_horizon - 1) * (seg_length-1)] = hl_goal[seg_length-1]
        LL_cond[-1] = hl_goal[seg_length-1]

        LL_action, LL_samples = LL_policy(LL_cond, batch_size=args.batch_size)
        actions = LL_samples.actions[0]
        sequence = LL_samples.observations[0]
    ...
    if t == 0: 
        ...
        renderer.composite(whole_fullpath, LL_samples.observations, ncol=1,  conditions=cond_plot)
