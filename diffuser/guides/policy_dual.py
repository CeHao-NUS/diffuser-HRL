...
class PolicyDual:
    ...
    def __call__(self, conditions_hl, conditions_ll, batch_size=1):

        # 1. initialize HL and LL diffusions
        conditions_hl_format = self._format_conditions(conditions_hl, batch_size)
        self.diffusion_model_hl(conditions_hl_format)
        conditions_ll_format = self._format_conditions(conditions_ll, batch_size)
        self.diffusion_model_ll(conditions_ll_format)

        done = False

        while not done:
            ll_variable = self.diffusion_model_ll.x
            
            cond_hl = xxx
            done_hl = self.diffusion_model_hl.forward_1step(conditions_hl_format)
            
            hl_variable = self.diffusion_model_hl.x
            cond_ll = xxx
            self.diffusion_model_ll.forward_1step(conditions_ll_format)
            
            done_ll = self.diffusion_model_ll.done

            done = done_hl and done_ll

        sample_hl = self.diffusion_model_hl.output()           
        sample_ll = self.diffusion_model_ll.output() 
